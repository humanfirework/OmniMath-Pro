...
export function SolverStepsView({
  ...
  return (
    ...
    >
      {/* 标题栏（点击标题可整体展开/收起） */}
      <div
        className="flex cursor-pointer select-none items-center justify-between px-3 py-2 border-b border-border/40 bg-background/40"
        onClick={() => setOpen((v) => !v)}
      >
        <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-foreground/85">
          <ChevronDown className={cn('size-3.5 text-primary transition-transform', !open && '-rotate-90')} />
          <ListOrdered className="size-3.5 text-primary" />
          {title}
          <span className="text-muted-foreground">({parsed.length})</span>
        </span>
        {open && defaultExpandedCount !== undefined && parsed.length > defaultExpandedCount && (
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
