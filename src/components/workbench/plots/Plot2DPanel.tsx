...
export function Plot2DPanel() {
  ...
  const effectiveY = effectiveUserView ? effectiveUserView.y : defaultView.y;
  ...
  // 切换到「手动」Y 范围时锁定当前视图，避免 effectiveUserView 为 null 时
  // 访问 .y 崩溃；同时让手动模式立即拥有具体的 Y 范围。
  const handleRangeModeChange = useCallback(
    (mode: RangeMode) => {
      setRangeMode(mode);
      if (mode === 'manual') {
        setUserView((prev) => ({
          view: prev?.view ?? { x: effectiveX, y: defaultView.y },
          plotCount: plots.length,
        }));
      }
    },
    [effectiveX, defaultView, plots.length],
  );
  ...
  const canvasProps: Plot2DCanvasProps = useMemo(
    ...
    [plots, theme, effectiveX, effectiveY, handleViewChange, handleResetView, handleInsertExample, showLegend, showMarkers, overlays, curveSpecs, curveSets],
  );
  ...
  return (
    <div className="plot-2d-panel flex h-full min-h-0 flex-col bg-background">
      ...
      />
      <div ref={canvasWrapperRef} className="plot-canvas-wrapper relative min-h-[240px] flex-1">
