 ...
 * 确保不遗漏任何对象轮廓。
 *
 * 独立成模块是为了打破与 `index.ts` 的循环依赖：`videoToCurves.ts` /
 * `visionWorkerClient.ts` / `visionWorker.ts` 需要调用 `imageToCurves`，
 * 而 `index.ts` 又要重新导出 `videoToCurves` 等，若彼此都走 barrel 会形成
 * 模块环，触发 webpack 的 “Circular dependency between chunks” 构建告警。
 * 这里让消费方直接 import 本模块，ring 即消除。
 */
import type {
  ImageDataLike,
  VisionOptions,
...
import { buildForegroundMask } from './fineOutline';
...
export function imageToCurves(
  ...
  if (opts.skeletonize) {
    ...
    if (opts.useMarchingSquares) {
      ...
      // 前景遮罩：复杂背景下仅保留主体内的边缘，抑制背景纹理噪点
      // （主体 = 面积最大且不接触图像边框的连通域，见 fineOutline.buildForegroundMask）
      if (opts.useForegroundMask) {
        const mask = buildForegroundMask(gray, w, h, opts.fgMaskDilation, 0.01);
        if (mask) {
          for (let i = 0; i < bin.length; i++) bin[i] = bin[i] && mask[i];
        }
      }
      // 注意：绝不对 1px 厚的 Canny/Sobel 边缘做「开运算」（opening）。
      // Canny 输出是单像素厚线；binaryMorphology('open') = 先腐蚀后膨胀，腐蚀用
      // 全方形结构元会把这些细线整条抹掉，导致「视频/图像转曲线提取出 0 条」。
      // 边缘图的降噪应只靠 removeSmallRegions 去掉孤立碎块即可（保线），
      // 因此这里不再调用 binaryMorphology。
  ...
  return {
    curves: capCurves(curves, opts.maxCurves),
    width: w,
    height: h,
    metadata: {
...
/**
 * 按 `maxCurves` 裁剪曲线数量（0/undefined 表示不限制）。
 * 面积越大越优先保留（主体轮廓），丢弃细碎噪点；保序稳定。
 */
function capCurves(curves: BezierPath[], maxCurves?: number): BezierPath[] {
  if (!maxCurves || maxCurves <= 0 || curves.length <= maxCurves) return curves;
  const withArea = curves.map((c, i) => ({ c, area: Math.abs(c.area ?? 0), i }));
  withArea.sort((a, b) => b.area - a.area || a.i - b.i);
  const picked = withArea.slice(0, maxCurves);
  picked.sort((a, b) => a.i - b.i); // 保持原始顺序
